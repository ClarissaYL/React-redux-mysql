...
import {saveResource, saveResourceToMysql,uploadRequest} from '../../actions/allResourceActions';
...
class ResourceForm extends Component {
    ...
    handleSubmit = (e) => {
        ...
        if (isValid) {
            ...
            const resourceId = website + '-' + headline; //TODO:应改为站名+站内数据库里的文件ID
            const allWeb = 1;

            if (file) {
                const formData = new FormData();
                formData.append('file', file);
                this.props.uploadRequest(formData).then(
                    () => {
                    },
...
export default connect(mapStateToProps, {saveResource, saveResourceToMysql, uploadRequest})(ResourceForm);